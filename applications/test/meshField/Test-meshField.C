...
void testIndexing(const fvMesh& fvMsh, const bool deep)
{
    ...
    {
        if (m1.direction()(i,j,k) != m1(i,j,k))
...
void testPrimitiveFunctions(const fvMesh& fvMsh, const bool deep)
{
    ...
    List<Type> mx(s1.numberOfDirections());

    forAll(m1.level(), d)
        mx[d] =
            pTraits<Type>::one*(d + cmptSum(m1.N(d)) - 3);
    ...
    List<Type> mn(s1.numberOfDirections());
...
void testFaceSpaceFunctions(const fvMesh& fvMsh)
{
    ...
    min(ls1, pTraits<FaceSpace<Type>>::one);
    min(pTraits<FaceSpace<Type>>::one, ls1);
...
void testVectorSpaceFunctions(const fvMesh& fvMsh, const bool deep)
{
    ...
    scalarList m1sm(m1.numberOfDirections(), Zero);
