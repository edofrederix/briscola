...
1. Numerical discretization schemes, algorithms, solvers and models are
...
1259-1288.]. Hysing's case consists of a 1x2 domain with a bubble of diameter 0.5
...
This variable needs to be set every time a new shell is started. It may
...
(there are two cases in total). If one wants to run the second Hysing case,
...
that this brick is decomposed into 2x2x1 processes. Thus, we need to run the case
...
before continuing. Remember to prepare the case again with the `prep.sh`
