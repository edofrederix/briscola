...
not encounter any errors: `Test metrics successful`. If an error is encountered,
...
```
Since the `Hysing.sh` script launches a large number of simulations, it is
...
automatically if enough cores are free, until all simulations have completed.
The number of cores can also be increased or decreased in `Hysing.job`.
