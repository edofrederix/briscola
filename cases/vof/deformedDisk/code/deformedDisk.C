...
defineTypeNameAndDebug(deformedDisk, 0);
...
tmp<colocatedScalarField> deformedDisk::alphaRef() const
{
    const label Nx = N_.x();
    const label Ny = N_.y();
    const label Nz = N_.z();
...
bool deformedDisk::read(const dictionary& dict)
{
