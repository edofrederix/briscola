...
defineTypeNameAndDebug(shearedSphere, 0);
...
tmp<colocatedScalarField> shearedSphere::alphaRef() const
{
    const label Nx = N_.x();
    const label Ny = N_.y();
    const label Nz = N_.z();
...
bool shearedSphere::read(const dictionary& dict)
{
