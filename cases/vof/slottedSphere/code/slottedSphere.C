...
defineTypeNameAndDebug(slottedSphere, 0);
...
tmp<colocatedScalarField> slottedSphere::alphaRef() const
{
    const label Nx = N_.x();
    const label Ny = N_.y();
    const label Nz = N_.z();
...
bool slottedSphere::read(const dictionary& dict)
{
