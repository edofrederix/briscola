...
defineTypeNameAndDebug(deformedSphere, 0);
...
tmp<colocatedScalarField> deformedSphere::alphaRef() const
{
    const label Nx = N_.x();
    const label Ny = N_.y();
    const label Nz = N_.z();
...
bool deformedSphere::read(const dictionary& dict)
{
