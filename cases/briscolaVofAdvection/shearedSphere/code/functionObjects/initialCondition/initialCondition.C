...
bool initialCondition::read(const dictionary& dict)
{
    if (runTime_.time().value() == 0.0)
    {
        ...
        forAllCells(alpha, i, j, k)
        {
            ...
            r = Foam::mag(vector(ccxy.x()-0.5,ccxy.y()-0.5,0));

            U(i,j,k) = vector
                (
                  Foam::cos(pi * runTime_.time().value() / T) * Foam::sqr(Foam::sin(pi * ccxy.x())) * Foam::sin(2 * pi * ccxy.y()),
                  - Foam::cos(pi * runTime_.time().value() / T) * Foam::sqr(Foam::sin(pi * ccxy.y())) * Foam::sin(2 * pi * ccxy.x()),
                  Foam::cos(pi * runTime_.time().value() / T) * Umax * Foam::sqr(1 - r/R)
                );

            alpha(i,j,k) = Foam::mag(ccxy - vector(0.5,0.75,0.25)) < 0.15;
...
bool initialCondition::execute()
{
    ...
    forAllCells(alpha, i, j, k)
    {
        ...
        U(i,j,k) = vector
            (
                Foam::cos(pi * (runTime_.time().value() + runTime_.deltaT().value()) / T) * Foam::sqr(Foam::sin(pi * ccxy.x())) * Foam::sin(2 * pi * ccxy.y()),
                - Foam::cos(pi * (runTime_.time().value() + runTime_.deltaT().value()) / T) * Foam::sqr(Foam::sin(pi * ccxy.y())) * Foam::sin(2 * pi * ccxy.x()),
                Foam::cos(pi * (runTime_.time().value() + runTime_.deltaT().value()) / T) * Umax * Foam::sqr(1 - r/R)
            );
...
bool initialCondition::end()
{
    if (Foam::mag(runTime_.time().value() - T) < 1e-12)
    {
        ...
        forAllCells(alpha, i, j, k)
        {
            ...
            L1Error += cv(i,j,k) * Foam::mag((Foam::mag(ccxy - vector(0.5,0.75,0.25)) < 0.15) - alpha(i,j,k));
