...
bool initialCondition::read(const dictionary& dict)
{
    ...
    if (runTime_.time().value() == 0.0)
    {
        ...
        forAllCells(alpha, i, j, k)
        {
            ...
            U(i,j,k) = vector
            (
                - 2 * Foam::cos(pi * runTime_.time().value() / T) * Foam::sqr(Foam::sin(pi * ccxy.x())) * Foam::sin(pi * ccxy.z()) * Foam::cos(pi * ccxy.z()),
                0,
                2 * Foam::cos(pi * runTime_.time().value() / T) * Foam::sqr(Foam::sin(pi * ccxy.z())) * Foam::sin(pi * ccxy.x()) * Foam::cos(pi * ccxy.x())
            );

            alpha(i,j,k) = Foam::mag(ccxy - vector(0.5,0,0.75)) < 0.15;
...
bool initialCondition::execute()
{
    ...
    forAllCells(U, i, j, k)
    {
        ...
        U(i,j,k) = vector
            (
                - 2 * Foam::cos(pi * (runTime_.time().value() + runTime_.deltaT().value()) / T) * Foam::sqr(Foam::sin(pi * ccxy.x())) * Foam::sin(pi * ccxy.z()) * Foam::cos(pi * ccxy.z()),
                0,
                2 * Foam::cos(pi * (runTime_.time().value() + runTime_.deltaT().value()) / T) * Foam::sqr(Foam::sin(pi * ccxy.z())) * Foam::sin(pi * ccxy.x()) * Foam::cos(pi * ccxy.x())
            );
...
bool initialCondition::end()
{
    if (Foam::mag(runTime_.time().value() - T) < 1e-12)
    {
        ...
        forAllCells(alpha, i, j, k)
        {
            ...
            L1Error += cv(i,j,k) * Foam::mag((Foam::mag(ccxy - vector(0.5,0,0.75)) < 0.15) - alpha(i,j,k));
