...
bool initialCondition::read(const dictionary& dict)
{
    ...
    if (runTime_.time().value() == 0.0)
    {
        ...
        forAllCells(alpha, i, j, k)
        {
            ...
            U(i,j,k) = vector
            (
                - 2 * Foam::cos(pi * (runTime_.time().value() + 0.5 * runTime_.deltaT().value()) / T) * Foam::sqr(Foam::sin(pi * ccxy.x())) * Foam::sin(pi * ccxy.y()) * Foam::cos(pi * ccxy.y()),
                2 * Foam::cos(pi * (runTime_.time().value() + 0.5 * runTime_.deltaT().value()) / T) * Foam::sqr(Foam::sin(pi * ccxy.y())) * Foam::sin(pi * ccxy.x()) * Foam::cos(pi * ccxy.x()),
                0
            );
        }
                            ...
                            for (int l = 0; l < 8; l++)
                            {
                                ...
                                alpha(i,j,k) += (1/(double(Nx * Ny * Nz) * cellSizes[0] * cellSizes[1] * cellSizes[2])) * 0.125
                                        * Foam::min(cellSizes[0] * cellSizes[1] * cellSizes[2], Foam::max(0,
                                        10 * (Foam::sqr(0.15) - Foam::sqr(Foam::mag(v - vector(0.5,0.75,0))))));
                            }
                        }
                    }
                }
        ...
        else
        {
            forAllCells(alpha, i, j, k)
            {
                ...
                for (int l = 0; l < 8; l++)
                {
                    ...
                    v.z() = 0;
                    alpha(i,j,k) += 0.125 * (Foam::mag(v - vector(0.5,0.75,0)) < 0.15);
                }
                TotalVolume += cv(i,j,k) * alpha(i,j,k);
            }
        }
        ...
        else
        {
            forAllCells(alpha, i, j, k)
            {
                ...
                for (int l = 0; l < 8; l++)
                {
                    vector v = vertex(i,j,k)[l];
                    v.z() = 0;
                    exactVol += 0.125 * (Foam::mag(v - vector(0.5,0.75,0)) < 0.15);
                }
