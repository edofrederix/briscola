#include "dampedStratifiedFlow.H"
...
defineTypeNameAndDebug(dampedStratifiedFlow, 0);

addToRunTimeSelectionTable
(
    functionObject,
    dampedStratifiedFlow,
    dictionary
);

bool dampedStratifiedFlow::read(const dictionary& dict)
{
    if (runTime_.time().value() == 0.0)
    {
        ...
        const meshField<faceVector,colocated> fc =
            alpha.fvMsh().metrics<colocated>().aos().faceCenters();
